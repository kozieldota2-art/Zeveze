  ...
  data: new SlashCommandBuilder()
    ...
    .addSubcommand(sub => sub
      ...
      .addStringOption(opt => opt
        .setName('comp')
        .setDescription('Nome da composição')
        .setRequired(true)
        .setAutocomplete(true)
      )
      .addStringOption(opt => opt
        .setName('nome')
        .setDescription('Nome da arma (ex: Hallowfall, Graveguard Sword, Blight...)')
        .setRequired(true)
        .setMaxLength(60)
      )
      .addStringOption(opt => opt
        .setName('role')
        .setDescription('Papel desta arma na comp')
        .setRequired(true)
        .addChoices(...ROLES.map(r => ({ name: r, value: r })))
      )
      .addStringOption(opt => opt
        .setName('build')
        .setDescription('Link da build (metabattle, albionbuilder, etc.)')
        .setRequired(false)
      )
    )
  ...
  async execute(interaction) {
    if (!isOfficer(interaction)) {
      return interaction.reply({
        content: '❌ Você precisa ser **Officer** para usar este comando.',
        ephemeral: true
      });
    }
    ...
    if (sub === 'adicionar') {
      ...
      if (!comp) {
        return interaction.reply({ content: `❌ Composição **${compName}** não encontrada.`, ephemeral: true });
      }
    ...
    if (sub === 'listar') {
      ...
      if (!weapons.length) {
        return interaction.reply({
          content: `📋 Nenhuma arma cadastrada na composição **${comp.name}**.\nUse \`/arma adicionar\` para começar.`,
          ephemeral: true
        });
      }
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(comp.description || '200b')
        .addFields(
          Object.entries(byRole).map(([role, ws]) => ({
            name: `${ROLE_EMOJI[role] || '🔹'} ${role}`,
            value: ws.map(w =>
              `• **${w.name}**${w.build_url ? ` — [Build](${w.build_url})` : ''}`
            ).join('\n'),
            inline: true
          }))
        )
    ...
    if (sub === 'remover') {
      ...
      if (!weapon) {
        return interaction.reply({ content: `❌ Arma **${weaponName}** não encontrada em **${comp.name}**.`, ephemeral: true });
      }

      db.removeWeapon(weapon.id);
      return interaction.reply({
        content: `✅ Arma **${weapon.name}** removida da composição **${comp.name}**.`,
        ephemeral: true
      });
    }
  },
  ...
  async autocomplete(interaction) {
    ...
    if (focused.name === 'nome') {
      ...
      const weapons = db.getWeaponsByComp(comp.id)
        ...
        .slice(0, 25)
        .map(w => ({ name: `${ROLE_EMOJI[w.role] || ''} ${w.name}`, value: w.name }));
